...
/** Kelompokkan transaksi sesuai filter dan kembalikan array data untuk LineChart */
const buildChartData = (transactions, filter) => {
  const now   = new Date()
  const today = new Date(now.getFullYear(), now.getMonth(), now.getDate())

  if (filter === '1_week') {
    // 7 hari terakhir, sumbu X = nama hari
  ...
  if (filter === '1_month') {
    // 1–31 tanggal bulan berjalan, sumbu X = tanggal
  ...
  if (filter === '1_year') {
    // 12 bulan tahun berjalan, sumbu X = nama bulan
    const year  = now.getFullYear()
...
export default function Pemasukan({ onOpenAddTransaction }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('pemasukan:updated', inc)
    return () => {
      window.removeEventListener('transactions:updated', inc)
      window.removeEventListener('pemasukan:updated', inc)
    }
  }, [])
  ...
  const loadTransactions = async () => {
    try {
      ...
      const safeData = data || []
      setTransactions(safeData.filter(t => t.type === 'Pemasukan'))
    } catch (err) {
      console.error(err)
  ...
  const totalIncome   = filteredTx.reduce((sum, t) => sum + (t.amount || 0), 0)
  // Dynamic matching: include any category that contains 'kas' or 'iuran' (case-insensitive)
  const totalIuran    = filteredTx.filter(t => {
    const categoryName = (t.category || '').toLowerCase()
    return categoryName.includes('kas') || categoryName.includes('iuran')
  }).reduce((sum, t) => sum + (t.amount || 0), 0)
  const totalSponsor  = filteredTx.filter(t => t.category === 'Sponsor').reduce((sum, t) => sum + (t.amount || 0), 0)

  const cardTitle = `Total Pemasukan ${FILTER_LABEL[timeFilter] ?? ''}`
  ...
  const recentIncome = approvedAndSortedTransactions.slice(0, 4)
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-md">
        <div>
          <h1 className="font-headline-lg-mobile md:font-headline-lg text-headline-lg-mobile md:text-headline-lg font-bold text-primary">
            Laporan Pemasukan
          </h1>
          <p className="font-body-md text-body-md text-on-surface-variant mt-xs">
            Ringkasan dana masuk dari iuran, sponsor, dan aktivitas Dutophy.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-gutter">
        <div className="bg-surface-container-lowest border border-outline-variant p-md rounded-xl flex flex-col justify-between relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10">
            <span className="material-symbols-outlined text-6xl">account_balance_wallet</span>
          </div>
          <h3 className="font-label-md text-label-md text-secondary uppercase mb-2">{cardTitle}</h3>
          <p className="font-headline-lg text-headline-lg font-bold text-primary mb-1">
            Rp {totalIncome.toLocaleString('id-ID')}
          </p>
          <div className="flex items-center gap-2 mt-2">
            <span className={`${isPositiveGrowth ? 'bg-green-900/40 text-green-400' : 'bg-error-container/40 text-error'} px-2 py-1 rounded font-label-md text-[10px] flex items-center gap-1`}>
        ...
        <div className="bg-surface-container-lowest border border-outline-variant p-md rounded-xl flex flex-col justify-between relative overflow-hidden">
          ...
          <h3 className="font-label-md text-label-md text-secondary uppercase mb-2">Iuran Kas</h3>
          <p className="font-headline-lg text-headline-lg font-bold text-primary mb-1">
            Rp {totalIuran.toLocaleString('id-ID')}
          </p>
          <p className="font-body-md text-[14px] text-secondary mt-2">{FILTER_LABEL[timeFilter]}</p>
        </div>
        <div className="bg-surface-container-lowest border border-outline-variant p-md rounded-xl flex flex-col justify-between relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10">
            <span className="material-symbols-outlined text-6xl">monetization_on</span>
          </div>
          <h3 className="font-label-md text-label-md text-secondary uppercase mb-2">Dana Sponsor / Eksternal</h3>
          <p className="font-headline-lg text-headline-lg font-bold text-primary mb-1">
            Rp {totalSponsor.toLocaleString('id-ID')}
          </p>
          <p className="font-body-md text-[14px] text-secondary mt-2">{FILTER_LABEL[timeFilter]}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-gutter">
        {/* Line Chart */}
        <div className="lg:col-span-2 bg-surface-container-lowest border border-outline-variant rounded-xl p-md flex flex-col h-full">
          <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-sm mb-md">
            <h3 className="font-headline-md text-headline-md font-bold text-primary">Grafik Pemasukan</h3>
            <select
              value={timeFilter}
            ...
            >
              ...
              <option value="1_year">Tahun Ini</option>
            </select>
          </div>
        ...
        <div className="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden flex flex-col">
          <div className="p-4 border-b border-outline-variant flex items-center bg-surface-container-low">
              ...
              <div key={index} className="flex items-center justify-between pb-3 border-b border-surface-variant last:border-0">
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <div className="min-w-0">
                    <p className="font-body-md text-body-md text-on-surface font-semibold truncate">{tx.description}</p>
                    <p className="font-body-md text-[14px] text-secondary">
                      {new Date(tx.createdAt).toLocaleDateString('id-ID')}
                    </p>
                  </div>
                </div>
                <span className="font-body-md text-body-md font-bold text-green-400 ml-2 flex-shrink-0">
                  +Rp {tx.amount.toLocaleString('id-ID')}
                </span>
              </div>
            )) : (
              <p className="text-secondary text-center py-4">Tidak ada transaksi</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden">
        ...
        <div className="p-md border-b border-outline-variant flex justify-between items-center bg-surface-container-low">
          <h3 className="font-headline-md text-headline-md font-bold text-primary">
            {showAll ? 'Semua Pemasukan' : 'Detail Pemasukan — 10 Terbaru'}
          </h3>
          <button
            type="button"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody
              className="font-body-md text-[14px] text-on-surface"
                  ...
                  <td colSpan="5" className="p-8 text-center">
                    <div className="mx-auto max-w-xl rounded-3xl border border-outline-variant bg-surface p-6">
                      ...
                      <p className="font-body-md text-body-md text-on-surface-variant">
                        Tambahkan transaksi lewat tombol Tambah Transaksi di sidebar untuk memulai pencatatan pemasukan.
                      </p>
                    </div>
                  </td>
                </tr>
